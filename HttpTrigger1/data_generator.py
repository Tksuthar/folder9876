...
class DataGenerator:
    ...
    def get_ambient_temp(self, temp):
    ...
    def loaded_model(self,fileloc):
        ...
        # loaded_model=load_model(fileloc)load_model(filepath=Path('./Image_Classifier.h5'))
        return loaded_model
    ...
    def data_generator_function(self, amb_temp):
        ...
        model=self.loaded_model(r"HttpTrigger1\my_model.h5")
        
        tf1=pd.DataFrame({'temp_lv_w1':t_lv1,
                        'temp_lv_w2':t_lv2,
                        'temp_hv':t_hv,
                        'OTI_temp':oti_t,})
        ...
        trans_data=pd.concat([tf1_c],axis=0)
        trans_json=trans_data.to_json(orient='records')
        return trans_json
